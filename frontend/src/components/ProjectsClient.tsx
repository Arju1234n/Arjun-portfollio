...
// Client component that receives server-fetched data and can update it
import React, { useState, useEffect } from 'react';
import { Github, ExternalLink, ArrowRight, Star } from 'lucide-react';
import Reveal from './Reveal';
import { apiFetch, type ApiProject } from '@/hooks/usePortfolioData';
      ...
      <article className="surface overflow-hidden hover-lift" style={{ minHeight: '260px' }}>
        <div className={`flex flex-col md:flex-row ${reverse ? 'md:flex-row-reverse' : ''} h-full`}>
          ...
          <div className="flex flex-col justify-between flex-1 p-7 md:p-8">
            <div>
              <p className="text-xs font-mono font-semibold tracking-widest uppercase mb-2" style={{ color: 'var(--text-3)', fontFamily: 'var(--font-mono, monospace)' }}>
                Project {String(index + 1).padStart(2, '0')}
              </p>
              <h3 className="text-2xl font-bold tracking-tight mb-3" style={{ color: 'var(--text-1)' }}>
                {project.title}
              </h3>
            ...
            <div className="flex items-center gap-4 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
              {project.githubUrl && (
                <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-semibold transition-colors duration-150" style={{ color: 'var(--text-3)' }} aria-label={`${project.title} GitHub repository`}>
                  <Github size={15} aria-hidden="true" />
                  Source
                </a>
              )}
              {project.liveUrl && (
                <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-semibold transition-colors duration-150" style={{ color: 'var(--accent)' }} aria-label={`${project.title} live demo`}>
  ...
  return (
    <Reveal>
      <article className="surface hover-lift p-5 flex items-start gap-4">
        <span className="flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: 'var(--surface-2)', color: 'var(--text-3)', fontFamily: 'var(--font-mono, monospace)' }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-1">
            ...
            <span className="flex-shrink-0 flex items-center gap-1 text-[10px] font-semibold" style={{ color: status.color }}>
          ...
          <div className="flex items-center justify-between">
            ...
            {project.githubUrl && (
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-3)' }} className="transition-colors duration-150" aria-label={`${project.title} GitHub`}>
...
export default function ProjectsClient({ initialProjects, initialError }: ProjectsClientProps) {
  ...
  const others   = projects.filter((p) => !p.featured && p.status === 'published');
    ...
    <section id="projects" className="section-pad bg-[var(--bg)]">
      <div className="container-lg">
        ...
        <Reveal>
          ...
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-2" style={{ color: 'var(--text-1)' }}>
            What I&apos;ve built
          </h2>
        ...
        {others.length > 0 && (
          <>
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-10">
              {others.map((p, i) => (
                <OtherCard key={p._id} project={p} index={i} />
              ))}
            </div>
          </>
        )}
        ...
        {/* GitHub CTA */}
        <Reveal>
          <a href="https://github.com/Arju1234n" target="_blank" rel="noopener noreferrer" className="btn-ghost group inline-flex">
